...
export const enableMouse = async (page: Page): Promise<void> => {
  await page.evaluateOnNewDocument((svgs) => {
    const moveCursor = (cursor: HTMLDivElement, event: MouseEvent): void => {
    ...
    const setCursorStyle = (
      ...
      const svg = svgs[cursorType] || svgs.default
